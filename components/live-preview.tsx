...
export default function LivePreview({ initialData }: { initialData: Data }) {
  ...
  useEffect(() => {
    ...
    const schedule = () => { if (document.visibilityState === 'visible') timer = window.setTimeout(async () => { await refresh(controller.signal); schedule() }, 30000) }
