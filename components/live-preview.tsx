...
const imageFor = (candidate: Candidate) => candidate.photoKetua || candidate.image || candidate.foto || candidate.photo || ''
...
export default function LivePreview({ initialData }: { initialData: Data }) {
  ...
  useEffect(() => { const timer = window.setInterval(refresh, 30000); return () => window.clearInterval(timer) }, [])
  ...
  return <main className="min-h-screen overflow-hidden bg-[#100d1f] text-white selection:bg-[#7657e8]">
    <header className="flex items-center justify-between border-b border-white/10 px-6 py-5 sm:px-10 lg:px-16">
      <div className="flex items-center gap-3"><img src="https://pemilos.osisaa16.my.id/logo-osis.png" alt="Logo OSIS" className="h-11 w-11 rounded-2xl object-cover" /><div><p className="text-sm font-bold tracking-wide">SMAZA<span className="text-[#a696ff]">16</span></p><p className="text-[10px] uppercase tracking-[.24em] text-white/45">Pemilihan Ketua OSIS</p></div></div>
    ...
    <section className="mx-auto max-w-[1500px] px-6 py-10 sm:px-10 lg:px-16 lg:py-14"><div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><div className="mb-4 flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-[#a696ff]"><Radio size={14} className="animate-pulse"/> Live results</div><h1 className="max-w-3xl text-4xl font-black tracking-tight sm:text-6xl">Hasil sementara<br/><span className="text-white/45">pemilihan OSIS.</span></h1></div><div className="rounded-2xl border border-white/10 bg-white/[.06] px-5 py-4"><p className="text-xs text-white/45">Total suara masuk</p><p className="mt-1 text-3xl font-black">{total.toLocaleString('id-ID')}</p><p className="mt-1 text-xs text-[#55d0b7]">{data.sessionStatus === 'open' ? 'Voting sedang berlangsung' : 'Sesi voting ditutup'}</p></div></div>
      <div className="grid gap-5 md:grid-cols-2">{candidates.map((candidate, index) => { const percent = total ? (candidate.count / total) * 100 : 0; const image = imageFor(candidate); return <article key={candidate.label} className={`relative overflow-hidden rounded-[28px] border p-5 transition ${index === 0 ? 'border-[#7657e8]/70 bg-[#211a42]' : 'border-white/10 bg-white/[.06]'}`}><div className="flex items-center gap-5"><div className="relative h-28 w-24 shrink-0 overflow-hidden rounded-2xl bg-white/10">{image ? <img src={image} alt={`Foto ${candidate.namaKetua || candidate.label}`} className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = 'none' }}/> : <div className="flex h-full items-center justify-center text-3xl font-black text-white/25">{(candidate.namaKetua || candidate.label).charAt(0)}</div>}<span className="absolute left-2 top-2 rounded-lg bg-black/40 px-2 py-1 text-[10px] font-bold">#{index + 1}</span></div><div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-[.18em]" style={{ color: candidate.color }}>Pasangan calon</p><h2 className="mt-2 truncate text-xl font-black sm:text-2xl">{candidate.label}</h2><p className="mt-1 truncate text-sm text-white/50">{candidate.namaKetua}{candidate.namaWakil ? ` & ${candidate.namaWakil}` : ''}</p></div><div className="text-right"><p className="text-3xl font-black">{candidate.count.toLocaleString('id-ID')}</p><p className="text-xs text-white/45">suara</p></div></div><div className="mt-6"><div className="mb-2 flex justify-between text-xs text-white/45"><span>Perolehan suara</span><strong className="text-white">{percent.toFixed(1)}%</strong></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${percent}%`, backgroundColor: candidate.color }}/></div></div>{index === 0 && <div className="absolute right-5 top-5 flex items-center gap-1.5 rounded-full bg-[#7657e8] px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider"><Trophy size={12}/> Unggul</div>}</article> })}</div>
      ...
      <footer className="mt-10 flex justify-between text-xs text-white/30"><span>Data diperbarui otomatis setiap 30 detik</span><span>SMA Negeri 16 Jakarta</span></footer>
    </section>
  </main>
}
